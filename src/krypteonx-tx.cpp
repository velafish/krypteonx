...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || gArgs.IsArgSet("-?") || gArgs.IsArgSet("-h") || gArgs.IsArgSet("-help")) {
        ...
        std::string strUsage = strprintf("%s krypteonx-tx utility version", PACKAGE_NAME) + " " + FormatFullVersion() + "\n\n" +
                               "Usage:\n"
                               "  krypteonx-tx [options] <hex-tx> [commands]  Update hex-encoded krypteonx transaction\n" +
                               "  krypteonx-tx [options] -create [commands]   Create hex-encoded krypteonx transaction\n" +
                               "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded krypteonx transaction
