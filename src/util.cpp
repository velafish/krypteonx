...
const char* const BITCOIN_CONF_FILENAME = "krypteonx.conf";
const char* const BITCOIN_PID_FILENAME = "krypteonxd.pid";
...
public:
    ...
    static inline std::pair<bool, std::string> GetArg(const ArgsManager& am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "krypteonxd -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\KrypteonxCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\KrypteonxCore
    // Mac: ~/Library/Application Support/KrypteonxCore
    // Unix: ~/.krypteonxcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "KrypteonxCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/KrypteonxCore";
#else
    // Unix
    return pathRet / ".krypteonxcore";
#endif
#endif
}
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty krypteonx.conf if it does not excist
...
std::string CopyrightHolders(const std::string& strPrefix, unsigned int nStartYear, unsigned int nEndYear)
{
    ...
    // Check for untranslated substitution to make sure Krypteonx Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Krypteonx Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + strprintf(" %u-%u ", 2021, nEndYear) + "The Krypteonx Core developers";
    }
