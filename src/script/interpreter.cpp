    ...
    try
    {
        while (pc < pend)
        {
            ...
            switch (opcode)
            {
                ...
                case OP_EQUAL:
                case OP_EQUALVERIFY:
                //case OP_NOKRYUAL: // use OP_NUMNOKRYUAL
                {
                    // (x1 x2 - bool)
                    if (stack.size() < 2)
                        return set_error(serror, SCRIPT_ERR_INVALID_STACK_OPERATION);
                    ...
                    // OP_NOKRYUAL is disabled because it would be too easy to say
                    // something like n != 1 and have some wiseguy pass in 1 with extra
                    // zero bytes after it (numerically, 0x01 == 0x0001 == 0x000001)
                    //if (opcode == OP_NOKRYUAL)
                    //    fEqual = !fEqual;
                    popstack(stack);
                    popstack(stack);
                ...
                case OP_0NOKRYUAL:
                {
                    ...
                    case OP_0NOKRYUAL:  bn = (bn != bnZero); break;
                ...
                case OP_MAX:
                {
                    ...
                    case OP_NUMNOKRYUAL:         bn = (bn1 != bn2); break;
