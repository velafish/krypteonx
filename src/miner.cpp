...
void static KrypteonxMiner(const CChainParams& chainparams)
{
    LogPrintf("KrypteonxMiner -- started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("krypteonx-miner");
    ...
    if (!EnsureWalletIsAvailable(pWallet, false)) {
        LogPrintf("KrypteonxMiner -- Wallet not available\n");
    }
#endif
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get()) {
                LogPrintf("KrypteonxMiner -- Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            ...
            LogPrintf("KrypteonxMiner -- Running miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
        ...
        LogPrintf("KrypteonxMiner -- terminated\n");
        throw;
    } catch (const std::runtime_error& e) {
        LogPrintf("KrypteonxMiner -- runtime error: %s\n", e.what());
        return;
    }
}

int GenerateKrypteonxs(bool fGenerate, int nThreads, const CChainParams& chainparams)
{
    ...
    for (int i = 0; i < nThreads; i++) {
        minerThreads->create_thread(boost::bind(&KrypteonxMiner, boost::cref(chainparams)));
    }
    return (numCores);
}
