...
# krypteonx_hash
RUN git clone https://github.com/Krypteonx-Coin/Core-Wallet_hash
RUN cd krypteonx_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} krypteonx
RUN useradd -u ${USER_ID} -g krypteonx -s /bin/bash -m -d /krypteonx krypteonx
...
RUN mkdir /krypteonx-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /krypteonx-src && \
  chown $USER_ID:$GROUP_ID /cache && \
  chown $USER_ID:$GROUP_ID /cache -R
WORKDIR /krypteonx-src
