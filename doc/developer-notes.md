...
-   [Development guidelines](#development-guidelines)
    -   [General Krypteonx Core](#general-krypteonx-core)
    -   [Wallet](#wallet)
    -   [General C++](#general-c)
...
Krypteonx Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/Krypteonx-Coin/Core-Wallet/blob/master/contrib/valgrind.supp))
...
$ valgrind -v --leak-check=full src/krypteonxd -printtoconsole
```
...
Krypteonx Core can be compiled with various "sanitizers" enabled, which add
...
However, in open source software such as Krypteonx Core, where everyone uses
...
pay attention to for reviewers of Krypteonx Core code.

## General Krypteonx Core
...
-   For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
    ...
    -   _Rationale_: Krypteonx Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against Krypteonx Core so that they can be integrated
...
-   Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
    ...
    -   _Rationale_: This is impossible to use with `krypteonx-cli`, and can be surprising to users.
...
-   Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
    ...
    -   _Rationale_: `krypteonx-cli` and the GUI debug console use this table to determine how to
...
-   Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
    ...
    -   _Rationale_: In previous versions of Krypteonx Core, the wallet was always
