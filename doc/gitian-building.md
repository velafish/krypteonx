...
_Setup instructions for a Gitian build of Krypteonx Core using a Debian VM or physical system._

Gitian is the deterministic build process that is used to build the Krypteonx
...
-   [Building Krypteonx Core](#building-krypteonx-core)
...
Clone the git repositories for Krypteonx Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/Krypteonx-Coin/Core-Wallet
...
in the Krypteonx Core repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

## Building Krypteonx Core

To build Krypteonx Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Krypteonx Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/krypteonx/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/Krypteonx-Coin/Core-Wallet
...
```bash
URL=https://github.com/crowning-/krypteonx.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit krypteonx=${COMMIT} --url krypteonx=${URL} ../krypteonx/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit krypteonx=${COMMIT} --url krypteonx=${URL} ../krypteonx/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit krypteonx=${COMMIT} --url krypteonx=${URL} ../krypteonx/contrib/gitian-descriptors/gitian-osx.yml
```

## Building fully offline

For building fully offline including attaching signatures to unsigned builds, the detached-sigs repository
and the krypteonx git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=bionic on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../krypteonx/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/krypteonx
SIGPATH=/some/root/path/krypteonx-detached-sigs

./bin/gbuild --url krypteonx=${BTCPATH},signature=${SIGPATH} ../krypteonx/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/krypteonx-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/krypteonx-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/krypteonx-osx-build.assert
```
...
[krypteonx/gitian.sigs](https://github.com/krypteonx/gitian.sigs/) repository, or if that's not possible to create a pull
request.
