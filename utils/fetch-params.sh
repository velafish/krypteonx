...
if [ "$uname_S" = "Darwin" ]; then
    PARAMS_DIR="$HOME/Library/Application Support/KrypteonxParams"
else
    PARAMS_DIR="$HOME/.krypteonx-params"
fi
...
DOWNLOAD_URL="https://github.com/Krypteonx-Coin/Core-Wallet/releases/tag/1.2.15.3"
...
# fetch methods can be disabled with KRY_DISABLE_SOMETHING=1
KRY_DISABLE_WGET="${KRY_DISABLE_WGET:-}"
#KRY_DISABLE_IPFS="${KRY_DISABLE_IPFS:-}"
KRY_DISABLE_CURL="${KRY_DISABLE_CURL:-}"

LOCKFILE=/tmp/fetch_params.lock

fetch_wget() {
    if [ -z "$WGETCMD" ] || ! [ -z "$KRY_DISABLE_WGET" ]; then
        return 1
    fi
...
#fetch_ipfs() {
#    if [ -z "$IPFSCMD" ] || ! [ -z "$KRY_DISABLE_IPFS" ]; then
#        return 1
#    fi
...
fetch_curl() {
    if [ -z "$CURLCMD" ] || ! [ -z "$KRY_DISABLE_CURL" ]; then
        return 1
    fi
...
Failed to fetch the Krypteonx parameters!
...
This script will fetch the Krypteonx SNARK parameters and verify their
...
EOF
    ...
    if ! [ -d "$PARAMS_DIR" ]; then
        ...
        cat >>"$README_PATH" <<EOF
This directory stores common Krypteonx SNARK parameters. Note that it is
